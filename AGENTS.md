...
📝 Planned only — spec exists in `PLAN.md`, nothing implemented yet. Do not assume these
...
|---|---|---|---|
| `README.md` | ✅ | Repo overview, tools list, pointer to this file | Onboarding / "what is this repo" |
| `PLAN.md` | 📝 (spec) | Full design spec for "Council" — the multi-LLM panel system (Codex + Cursor fan-out, cross-pollination, governed ledger) | Any task that touches multi-model orchestration — read this in full before building |
| `.claude/skills/ideate/SKILL.md` | ✅ | Operator skill: routes brainstorm/validate/research/combine requests to the right toolkit recipe, drives diverge→converge sessions | User wants to brainstorm, validate, research a space, or combine ideas |
...
| `creative-thinking-toolkit/05-prompt-library.md` | ✅ | Paste-ready prompts + 4 composed session recipes (Full Ideation Run, Kill-the-Idea, Research-a-Space, Deep-Tunnel Sweep) | Running an actual session — this is the operational core |
...
| `council/` | 📝 | Session workspace for the panel system (per-session folders: brief, deep-research, divergent seeds, panel outputs, cross-pollination, synthesis, ledger) | Not yet created — see `PLAN.md` |
| `.claude/skills/council/`, `ask-cursor/`, `council-setup/` | 📝 | Orchestrator skill, Cursor bridge, local setup checker | Not yet created — see `PLAN.md` |
| `scripts/cursor-agent.sh`, `scripts/cursor-panel.sh` | 📝 | Single-model and parallel-fanout wrappers around `cursor-agent` | Not yet created — see `PLAN.md` |
| `.codex/config.toml` | 📝 | Default model/effort for the Codex plugin | Not yet created |
...
- Both are wired into the **Council** design in `PLAN.md`, phase 1 (ad-hoc, you drive it from local Claude Code).
...
2. If the task touches multi-model orchestration, read `PLAN.md` in full first — it's the authoritative spec, this file is just the map.
