...
Opus reads **only the model outputs listed in `panel/outputs.manifest`** (Cursor seats from `cursor-panel.sh`, plus any Codex/other filenames the operator appended). This is recombination, not averaging.

Do **not** read `panel/README.md`, `panel/outputs.manifest` itself as a seat, or anything under `prompts/`. If the manifest is missing, use `panel/*.md` excluding `README.md` — still never prompts.
